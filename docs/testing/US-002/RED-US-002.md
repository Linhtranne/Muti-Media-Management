# RED-US-002: Airtable Approved Webhook Workflow Trigger

## RED Evidence

This is a retrofit evidence file. Original RED output was not captured at implementation time.

Current gap: US-002 predates the AI-SDLC completion gate or was completed before the repository required story-level RED artifacts.

Follow-up: future behavior changes for US-002 must capture real RED before production code changes.

## Failing Test / Check

- Retrofit gate failure: `npm run ai-sdlc:validate -- US-002` failed before this documentation retrofit because required AI-SDLC artifacts or headings were missing.
...
```powershell
npm run ai-sdlc:validate -- US-002
```
...
After retrofit, `npm run ai-sdlc:check -- US-002` should pass artifact existence, required headings, approved status, and AC traceability checks.
