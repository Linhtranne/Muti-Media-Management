export interface SecretStore {
  ...
  resolveSecretForChannel(workspaceId: string, channelAccountId: string): Promise<string>;
}
...
export class EnvSecretStore implements SecretStore {
  ...
  async resolveSecretForChannel(_workspaceId: string, _channelAccountId: string): Promise<string> {
    const mockVal = process.env.TIKTOK_ACCESS_TOKEN || "mock-tiktok-token";
    return Promise.resolve(mockVal);
  }
