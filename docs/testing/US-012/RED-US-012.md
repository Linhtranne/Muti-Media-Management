# RED-US-012: Basic Campaign Reporting

## RED Evidence

This is a retrofit evidence file. Original RED output was not captured at implementation time.

Current gap: US-012 predates the AI-SDLC completion gate or was completed before the repository required story-level RED artifacts.

Follow-up: future behavior changes for US-012 must capture real RED before production code changes.

## Failing Test / Check

- Retrofit gate failure: `npm run ai-sdlc:validate -- US-012` failed before this documentation retrofit because required AI-SDLC artifacts or headings were missing.
...
```powershell
npm run ai-sdlc:validate -- US-012
```
...
After retrofit, `npm run ai-sdlc:check -- US-012` should pass artifact existence, required headings, approved status, and AC traceability checks.
