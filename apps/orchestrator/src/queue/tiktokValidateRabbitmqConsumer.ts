...
import { PublishTiktokRequestedEventSchema } from "@mediaops/shared-contracts";
import type { Logger } from "../lib/logger.js";
import type { TiktokValidateWorker } from "../workers/tiktokValidateWorker.js";

export interface TiktokValidateQueueConsumerChannel {
...
export interface TiktokValidateQueueConnection {
  createConfirmChannel(): Promise<TiktokValidateQueueConsumerChannel>;
  close(): Promise<void>;
}

export interface TiktokValidateQueueConsumer {
  start(): Promise<void>;
  stop(): Promise<void>;
}
...
const queue = "publish.tiktok.requested";
const routingKey = "publish.tiktok.requested";
const dlqQueue = "publish.tiktok.requested.dlq";
const connectRabbitMq = amqp.connect as (url: string) => Promise<TiktokValidateQueueConnection>;
...
export async function handleTiktokValidateQueueMessage(
  channel: TiktokValidateQueueConsumerChannel,
  worker: Pick<TiktokValidateWorker, "processQueueMessage">,
  logger: Logger,
  ...
  try {
    ...
    const validation = PublishTiktokRequestedEventSchema.safeParse(rawPayload);
    ...
    if (result.action === "nack_requeue") {
      if (retryCount >= MAX_RETRIES) {
        logger.error("TikTok Validate worker requested requeue, but max retries exceeded, routing to DLQ", { messageId, status: result.status });
        ...
        logger.warn("Requeueing TikTok Validate message with backoff", {
          messageId,
    ...
    logger.error("Unhandled exception in TikTok Validate consumer loop", {
      messageId,
      error: error instanceof Error ? error.message : String(error)
    });
    if (retryCount >= MAX_RETRIES) {
      logger.error("TikTok Validate max retries exceeded on unhandled exception, routing to DLQ", { messageId });
...
export async function createTiktokValidateRabbitMqConsumer(
  rabbitmqUrl: string,
  worker: TiktokValidateWorker,
  logger: Logger
): Promise<TiktokValidateQueueConsumer> {
  let connection: TiktokValidateQueueConnection | null = null;
  let channel: TiktokValidateQueueConsumerChannel | null = null;
  let isStopping = false;

  return {
    async start(): Promise<void> {
      logger.info("Initializing TikTok Validate RabbitMQ consumer...");
      ...
      await channel.consume(queue, async (msg: amqp.ConsumeMessage | null) => {
        ...
        await handleTiktokValidateQueueMessage(channel, worker, logger, msg, () => isStopping);
      });
    },
    ...
    async stop(): Promise<void> {
      ...
      isStopping = true;
      logger.info("Stopping TikTok Validate RabbitMQ consumer...");

      if (channel) {
        await channel.close().catch((error) => { logger.error("Error closing TikTok Validate RabbitMQ channel", { error: String(error) }); });
      }
      if (connection) {
        await connection.close().catch((error) => { logger.error("Error closing TikTok Validate RabbitMQ connection", { error: String(error) }); });
      }
    }
  };
}
