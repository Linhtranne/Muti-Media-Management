...
import { SlackCommandActionEventSchema } from "@mediaops/shared-contracts";
import type { Logger } from "../lib/logger.js";
import type { SlackPostApprovalWorker } from "../workers/slackPostApprovalWorker.js";
...
export interface SlackCommandQueueConsumer {
  start(): Promise<void>;
  stop(): Promise<void>;
}

export function createSlackCommandRabbitmqConsumer(
  rabbitmqUrl: string,
  worker: SlackPostApprovalWorker,
  logger: Logger,
  workspaceId: string
): SlackCommandQueueConsumer {
  const queue = "slack.post_approval.requested";
  const dlq = "slack.post_approval.requested.dlq";
    ...
    async start() {
      ...
      await activeChannel.consume(queue, (msg: amqp.ConsumeMessage | null) => {
        ...
        try {
          ...
          const validatedEvent = SlackCommandActionEventSchema.parse(parsedBody);

          if (validatedEvent.workspace_id !== workspaceId) {
            logger.warn("Ignoring Slack command message for different workspace", {
              messageId,
          ...
          if (result.action === "ack") {
            ...
            if (retryCount >= MAX_RETRIES) {
              logger.error("Slack command max retries exceeded, routing to DLQ", { messageId, status: result.status });
              activeChannel.nack(msg, false, false);
            } else {
              const backoffMs = (2 ** retryCount) * 1000;
              logger.warn("Requeueing Slack command message with backoff", {
                messageId,
              ...
              const retryQueueName = `slack.post_approval.retry.${backoffMs}`;
            ...
            logger.error("Slack command routing to DLQ", { messageId, status: result.status });
            activeChannel.nack(msg, false, false);
          }
        } catch (error) {
          logger.error("Failed to parse Slack command queue message", {
            messageId,
      ...
      logger.info(`Started Slack command consumer for workspace ${workspaceId} on queue ${queue}`);
    },

    async stop() {
      if (channel) await channel.close();
      if (connection) await connection.close();
      logger.info(`Stopped Slack command consumer on queue ${queue}`);
    }
  };
}
